...
public class PlaceableFactory
{
    public Placeable CreateHouse()
    {
        ...
        Placeable house = new Placeable(PlaceableType.HOUSE,input, output,10);
        return house;
    }
    ...
    public Placeable CreateField()
    {
        ...
        int[] output = new int[Enum.GetNames(typeof(ResourceType)).Length];
        input[(int)ResourceType.MONEY] = 2;
        output[(int)ResourceType.HOP] = 2;
        Placeable field = new Placeable(PlaceableType.FIELD,input, output,10);
        return field;
    }
    ...
    public Placeable CreateIceUsine()
    {
        int[] input = new int[Enum.GetNames(typeof(ResourceType)).Length];
        int[] output = new int[Enum.GetNames(typeof(ResourceType)).Length];
        output[(int)ResourceType.ICE] = 2;
        Placeable ice_usine = new Placeable(PlaceableType.ICE_USINE,input, output,10);
        return ice_usine;
    }
